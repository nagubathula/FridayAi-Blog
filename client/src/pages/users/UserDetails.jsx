...
import BlogsCard2 from "@/components/ui/BlogsCard2";
...
const UserDetails = () => {
  ...
  return loading ? (
    <div className="flex justify-center mt-36 max-w-2xl">
    ...
    <div className="flex items-center justify-center my-24 sm:my-36">
      {data && !isError ? (
        <div className="w-[fit-content] flex flex-col gap-10 items-center">
          ...
          <div
            className={`max-w-3xl flex justify-${
          ...
          {data.blogs.length > 0 ? (
            <div className="max-w-3xl flex flex-col justify-items-center gap-8">
              {data.blogs.map((items) => {
                return <BlogsCard2 key={items._id} data={items} />;
              })}
            </div>
          ) : (
            <h1>
